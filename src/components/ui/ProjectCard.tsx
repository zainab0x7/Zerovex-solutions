  ...
  return (
    <>
      <div
        ...
        data-cursor="project"
        className="corp-card cursor-pointer overflow-hidden group flex flex-col justify-between h-full bg-black border border-zinc-800 hover:border-[#E31B2B]/60 transition-all duration-300 transform group-hover:-translate-y-1 hover:shadow-[0_0_30px_rgba(227,27,43,0.15)]"
      >
        {/* Card Top Vector Visual Header */}
        <div className="bg-zinc-950 p-6 border-b border-zinc-800 relative overflow-hidden">
          <div className="flex items-center justify-between pb-4">
            <span className="text-[11px] font-mono font-semibold uppercase tracking-wider text-[#E31B2B] px-2.5 py-1 rounded bg-red-950/40 border border-red-900/50">
              {category}
            </span>
            <div className="p-2 rounded bg-zinc-900 border border-zinc-800 text-zinc-300 group-hover:text-white group-hover:bg-[#E31B2B] group-hover:border-[#E31B2B] transition-all">
        ...
        <div className="p-6 space-y-4 flex-1 flex flex-col justify-between bg-black">
          ...
          <div className="pt-4 border-t border-zinc-800 flex flex-wrap gap-1.5">
            {servicesUsed.slice(0, 3).map((s, i) => (
              <span key={i} className="text-[10px] font-medium px-2 py-0.5 rounded bg-zinc-900 text-zinc-300 border border-zinc-800">
                {s}
              </span>
            ))}
          </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
          <div className="bg-black border border-zinc-800 rounded-xl max-w-3xl w-full p-6 sm:p-10 space-y-8 relative max-h-[90vh] overflow-y-auto text-left shadow-2xl">
            
            {/* Close Button */}
            <button
              onClick={() => setIsOpen(false)}
              className="absolute top-6 right-6 p-2 rounded bg-zinc-900 text-zinc-400 hover:text-white border border-zinc-800 transition-colors"
            >
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs text-zinc-400 pt-1 font-mono">
                <span>Client: <strong className="text-white">{client}</strong></span>
                <span>•</span>
                <span>Industry: <strong className="text-white">{industry}</strong></span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Challenge */}
              <div className="bg-zinc-950 p-6 rounded-lg border border-zinc-800 space-y-3">
              ...
              <div className="space-y-3">
                ...
                <ul className="space-y-2 text-xs text-zinc-300">
                  {designHighlights.map((d, i) => (
                    <li key={i} className="flex items-center gap-2 bg-zinc-900 p-2.5 rounded border border-zinc-800">
            ...
            {/* Result Box */}
            <div className="bg-red-950/30 p-6 rounded-lg border-l-4 border-l-[#E31B2B] border border-red-900/40 space-y-2">
            ...
            <div className="pt-4 flex items-center justify-between border-t border-zinc-800">
              <span className="text-xs text-zinc-400">ZeroVex Digital Engineering</span>
              <button
                onClick={() => setIsOpen(false)}
